...
class DWCNT:
    ...
    def _N_j(self, j, m, k, omega):
        """N_1, N_2 (Eq. 19 of arXiv:2401.08334), needed to weight each
        wall's contribution.
        ...
        SECOND BUG (fixed): B'_j = -n0 (k^2+m^2/a_j^2) 2 pi Q g(aj,r0) e^{-imphi0}
        (Eq. 10) has NO extra factor of a_j, and carries a leading minus
        sign -- both were missing below (the a_j factor matters, not just
        an overall scale, since B1p and B2p use different a_j and combine
        asymmetrically in N1/N2). Caught by validating against an
        isolated-resonance residue calculation (matches to ~0.4%).
        """
        def Bp(jj):
            aj = self.a[jj]
            q2 = k**2 + m**2 / aj**2
            return -self.n0 * q2 * 2 * np.pi * self.Q * _g(aj, self.r0, m, k) * \
        ...
        for m in range(-m_max, m_max + 1):
            for sign in (+1, -1):
                for km in self.resonances(sign, abs(m), k_max=k_max):
                    ...
                    if dZdk == 0:
                        continue
                    # D_m^mp(k_m^pm, k_m^pm v) at gamma=0 is just the
                    # OTHER branch's Z function evaluated at this k -- the
                    # residue formula (Eq. 24-25) divides by it; this was
                    # missing before (a real bug: without it, amplitudes
                    # are wrong by a mode/velocity-dependent factor, not
                    # just an overall constant).
                    D_other = self._Z_pm(-sign, abs(m), km)
                    if D_other == 0:
                        continue
                    omega = km * self.v
                    phase = np.exp(1j * m * (phi - self.phi0))
                    prefac_z = km / (2 * np.pi)**2 / (abs(dZdk) * D_other)
                    prefac_r = 1.0 / (2 * np.pi)**2 / (abs(dZdk) * D_other)
